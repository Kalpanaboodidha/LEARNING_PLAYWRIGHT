...
Created by: Pramod Dutta SDET Manager at Tekion | Ex BrowserStack | CSM® Certified Scrum Master 10+ Years in Software Testing & Test Automation | Mentored 10,000+ Students Founder — TheTestingAcademy.com LinkedIn — linkedin.com/in/pramoddutta YouTube — 150K+ Subscribers | Instagram — 100K+ Followers
...
What is GitHub MCP and Why QA Engineers Need It
Prerequisites & Environment Setup
Installing GitHub MCP in Antigravity (Step-by-Step)
Generating a GitHub Personal Access Token (PAT)
Verifying the Connection & Available GitHub Tools
GitHub MCP Tools — Complete Reference with QA Prompts
Real QA Workflows — Issues, PRs, CI/CD, Code Review
Multi-Agent GitHub Workflows
Troubleshooting GitHub MCP in Antigravity
Part B: VS Code with GitHub Copilot (Modules 10–18)
What is GitHub Copilot and Why QA Engineers Need It
Prerequisites & Installation in VS Code
Copilot Modes — Ask, Edit, and Agent Mode Explained
Keyboard Shortcuts & Slash Commands for QA
Context References — #file, @workspace, @terminal
Custom Instructions — copilot-instructions.md for QA
Agent Mode — Autonomous Multi-File Test Generation
Copilot Skills & Custom Agents for QA
...
Find a bug during testing
Switch to browser → open GitHub
Navigate to the repo → click "New Issue"
Manually type the bug title, description, steps, screenshots
Assign labels, milestone, assignee
Switch back to IDE → continue testing
With GitHub MCP inside Antigravity:

Find a bug during testing
Type: "Create a GitHub issue for the login page timeout bug I just found"
Done. The agent creates the issue with title, description, labels, and assignee.
...
Requirement	Details
Google Antigravity	Download from antigravity.google. macOS Monterey+, Windows 10+, or Linux. Free public preview with personal Gmail.
...
Download the .dmg from antigravity.google
Drag Antigravity to Applications
Open and sign in with your Gmail account
Select your preferred editor layout
Windows:

Download the .exe installer from antigravity.google
Run the installer and follow prompts
Sign in with your Gmail account
Linux:

Download the .AppImage or .deb from antigravity.google
Make executable: chmod +x Antigravity-*.AppImage
Run and sign in
...
If a token leaks, revoke it immediately at github.com/settings/tokens
...
Formats your description into a proper bug report
Calls create_issue with structured title, body, labels, and assignee
...
Close and fully reopen Antigravity (not just reload — quit and relaunch)
Check JSON syntax — a missing comma or bracket breaks everything:
# Validate your JSON
cat ~/.gemini/antigravity/mcp_config.json | python3 -m json.tool
Verify you used serverUrl not url — Antigravity requires serverUrl for HTTP servers
Check token validity — go to github.com/settings/tokens and verify your token hasn't expired
...
Ensure Docker is running — look for the Docker whale icon in your system tray
...
Regenerate your PAT — tokens expire, and the old one may be revoked
Check token permissions — ensure Issues, Pull Requests, Contents, and Metadata are enabled
Check for extra spaces — the token in mcp_config.json should have no leading/trailing spaces
Bearer prefix — ensure your Authorization header reads "Bearer ghp_xxxxx..." (with "Bearer " prefix and a space)
...
11.1 What You Need
Requirement	Details
VS Code	Version 1.99 or later. Download from code.visualstudio.com
...
Check Copilot status — look at the Status Bar icon. If it shows a warning, click it for details.
Verify sign-in — Ctrl+Shift+P → type GitHub Copilot: Sign In
Check language is enabled — Copilot can be disabled for specific languages. Go to Settings → search "copilot" → ensure your language isn't in the disabled list.
Check monthly limit — Free plan users may have exhausted their monthly quota.
Restart VS Code — sometimes a simple restart fixes connection issues.
18.2 Agent Mode Not Available
Symptom: No "Agent" option in the mode dropdown.

Fixes:

Update VS Code — Agent mode requires VS Code 1.99+. Check: Help > About.
Enable the setting:
{ "chat.agent.enabled": true }
Switch to Pre-Release extension — Go to Extensions → GitHub Copilot → click "Switch to Pre-Release Version" → restart.
Check subscription — Agent mode features may require Copilot Pro.
18.3 "Copilot could not connect to server"
Symptom: Network error in Copilot chat.

Fixes:

Check internet connection — Copilot requires internet access
Check proxy/firewall — ensure *.github.com and *.githubcopilot.com are not blocked
Sign out and sign back in:
Click the Accounts icon in the Activity Bar (left sidebar)
Select "Sign out"
Sign in again with your GitHub credentials
Check VPN — some VPNs block Copilot. Try disconnecting.
...
Create copilot-instructions.md (Module 15) — explicitly state your framework preferences
Be explicit in prompts — "Using Selenium 4 with Java 17 and TestNG, write..."
Open relevant files — Copilot uses your open tabs as context. Open your pom.xml or existing test files.
Use #file references — #file:pom.xml I want tests compatible with this project
18.6 Agent Mode Goes Off Track
Symptom: Agent keeps running commands that don't make sense or installs wrong dependencies.

Fixes:

Be more specific — vague prompts lead to wrong assumptions
Provide context — reference your config files: #file:playwright.config.ts
Intervene early — if the agent starts going wrong, hit Escape and redirect
Use the Undo button — "Undo Last Edit" in the Chat view title bar reverts the last change
Start a new session — Cmd+N / Ctrl+N to start fresh if context is too confused
18.7 Slow Suggestions or Timeouts
Fixes:

Close unnecessary tabs — too many open files slow down Copilot context
Reduce workspace size — add large folders (node_modules, target, build) to .gitignore
Switch models — in Chat, use the model dropdown to try a faster model
